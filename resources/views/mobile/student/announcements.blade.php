        ...
        <div class="ann-sheet-overlay" id="annOverlay{{ $a->id }}" onclick="closeAnn({{ $a->id }})" style="display: none; position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.4); z-index: 1000; align-items: flex-end;">
            <div class="ann-sheet" onclick="event.stopPropagation()" style="background: #fff; width: 100%; border-radius: 24px 24px 0 0; padding: 24px; max-height: 90vh; overflow-y: auto; transition: transform 0.3s cubic-bezier(0.4, 0, 0.2, 1); transform: translateY(100%);">
                ...
                <div style="border-top: 1px solid #f1f5f9; padding-top: 20px; margin-bottom: 20px;">
                    <div style="font-size:0.95rem; font-weight:700; color:#1e293b; margin-bottom:14px;">
                    ...
                    <div style="display:flex; flex-direction:column; gap:12px;">
                        ...
                        <div style="display:flex; gap:10px;">
                            <div style="width:34px; height:34px; border-radius:50%; background:#f1f5f9; color:#94a3b8; display:flex; align-items:center; justify-content:center; flex-shrink:0; font-size:1rem;">
                                <i class="bi bi-person-circle"></i>
                            </div>
                            <div style="flex:1;">
                                <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:4px;">
                                    <span style="font-size:0.78rem; font-weight:700; color:#1e293b;"><i class="bi bi-person-fill-lock"></i> Anonymous Student</span>
                                    <span style="font-size:0.68rem; color:#94a3b8;">{{ $c->created_at?->diffForHumans() }}</span>
                                </div>
                                <div style="background:#f8fafc; border-radius:12px; padding:10px 14px; font-size:0.85rem; color:#334155; line-height:1.6;">{!! nl2br(e($c->comment)) !!}</div>
                            </div>
                        </div>
                        @empty
                        <div style="text-align:center; padding:12px; color:#94a3b8; font-size:0.82rem;">No comments yet. Be the first to help!</div>
                        @endforelse
                    </div>
                </div>
                @endif
