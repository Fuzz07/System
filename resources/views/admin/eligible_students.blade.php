...
<div class="page-header">
    ...
    <div class="d-flex gap-2">
        <button class="btn btn-outline-primary d-flex align-items-center gap-2" data-bs-toggle="modal" data-bs-target="#importModal" style="border-radius:var(--radius-sm);font-weight:600;font-size:.85rem;">
...
<div class="row g-3 mb-4">
    <div class="col-md-4">
        <div class="card border-0 shadow-sm h-100" style="border-radius:var(--radius);background:linear-gradient(135deg,#1e3a5f,#2563eb);">
            <div class="card-body p-4 d-flex align-items-center gap-3">
                ...
                <div>
                    ...
                    <div style="font-size:.8rem;color:rgba(255,255,255,.75);font-weight:500;">Total Eligible Emails</div>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="col-md-4">
        <div class="card border-0 shadow-sm h-100" style="border-radius:var(--radius);background:#fff;">
            <div class="card-body p-4 d-flex align-items-center gap-3">
                ...
                <div>
                    <div style="font-size:1.7rem;font-weight:800;color:#1e293b;line-height:1;">{{ $total - $registered }}</div>
...
<div class="alert border-0 mb-4 d-flex align-items-start gap-3" style="background:#eff6ff;border-radius:var(--radius);padding:16px 20px;">
    ...
    <div>
        ...
        <div style="font-size:.82rem;color:#475569;margin-top:2px;">Only MS accounts in this list can complete student registration. To disable this, set <code>ENFORCE_ELIGIBILITY_WHITELIST=false</code> in your <code>.env</code>.</div>
    </div>
</div>
@else
<div class="alert border-0 mb-4 d-flex align-items-start gap-3" style="background:#fef2f2;border-radius:var(--radius);padding:16px 20px;">
    ...
    <div>
        ...
        <div style="font-size:.82rem;color:#475569;margin-top:2px;">Any <code>@mcclawis.edu.ph</code> account can currently register. Set <code>ENFORCE_ELIGIBILITY_WHITELIST=true</code> in <code>.env</code> to enforce the whitelist.</div>
    </div>
</div>
@endif
...
<div class="card mb-4"><div class="card-body-custom">
    <form method="GET" class="row g-2 align-items-end">
        <div class="col-md-5"><input type="text" name="search" class="form-control-custom" placeholder="Search by email or name..." value="{{ $search }}"></div>
        <div class="col-md-3">
        ...
        <div class="col-md-4 d-flex gap-2">
            <button type="submit" class="btn-primary-custom flex-fill justify-content-center"><i class="bi bi-search"></i></button>
            @if($search || $department)<a href="{{ route('admin.eligible_students.index') }}" class="btn btn-outline-secondary">Reset</a>@endif
        </div>
    </form>
</div></div>
...
<div class="d-flex justify-content-end mb-2">
    <a href="data:text/csv;charset=utf-8,email%2Cstudent_name%2Cdepartment%2Cyear_level%0Ajuan.dela.cruz%40mcclawis.edu.ph%2CJuan%20Dela%20Cruz%2CBSIT%2C3rd%20Year"
       ...
       class="btn btn-sm btn-outline-secondary d-flex align-items-center gap-1" style="font-size:.78rem;border-radius:6px;">
        ...
        <table class="table-custom">
            ...
            <tr>
                ...
                <td>
                    ...
                    @else
                        <span class="badge bg-warning text-dark" style="font-size:.7rem;border-radius:6px;padding:4px 10px;">
                ...
                <td>
                    <form method="POST" action="{{ route('admin.eligible_students.destroy', $e) }}" class="d-inline" onsubmit="return confirm('Remove {{ $e->email }} from the eligible list?')">
                        @csrf
                        @method('DELETE')
            ...
            <tr>
                <td colspan="7" class="text-center py-5 text-muted">
                    ...
                    <p class="text-muted mb-0 mt-1" style="font-size:.85rem;">Use the "Add Email" or "Import CSV" buttons above to get started.</p>
                </td>
            </tr>
            @endforelse
            </tbody>
    ...
    <div class="modal-dialog modal-md">
        <div class="modal-content" style="border-radius:var(--radius);border:none;box-shadow:0 10px 25px rgba(0,0,0,.12);">
            <div class="modal-header modal-header-custom">
                <h5 class="modal-title" style="font-weight:700;"><i class="bi bi-plus-circle"></i> Add Eligible Student</h5>
            ...
            <form method="POST" action="{{ route('admin.eligible_students.store') }}">
                ...
                <div class="modal-body p-4">
                    <div class="mb-3">
                        ...
                        <input type="email" name="email" class="form-control-custom" placeholder="juan.delacruz@mcclawis.edu.ph" required>
                    ...
                    <div class="row g-2 mb-3">
                        <div class="col-md-6">
                            ...
                            <input type="text" name="student_name" class="form-control-custom" placeholder="Juan Dela Cruz">
                        </div>
                        <div class="col-md-6">
                    ...
                    <div class="mb-3">
                        <label class="form-label-custom">Notes <span class="text-muted" style="font-weight:400;font-size:.78rem;">(optional)</span></label>
                        <textarea name="notes" class="form-control-custom" rows="2" placeholder="Any notes about this student..." style="resize:vertical;"></textarea>
                    </div>
                </div>
    ...
    <div class="modal-dialog modal-md">
        <div class="modal-content" style="border-radius:var(--radius);border:none;box-shadow:0 10px 25px rgba(0,0,0,.12);">
            ...
            <form method="POST" action="{{ route('admin.eligible_students.import') }}" enctype="multipart/form-data">
                @csrf
                <div class="modal-body p-4">
                    <div class="mb-3 p-3" style="background:#f8fafc;border-radius:var(--radius-sm);border:1px dashed #cbd5e1;">
                        <div style="font-size:.82rem;font-weight:700;color:#1e293b;margin-bottom:8px;"><i class="bi bi-info-circle text-primary me-1"></i>Expected CSV Format</div>
...
maria.santos@mcclawis.edu.ph,Maria Santos,BSED,2nd Year</code>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="file" name="csv_file" id="csv_file" accept=".csv,.txt" class="form-control-custom" required>
                    </div>
                </div>
                <div class="modal-footer border-0 pt-0">
                    <button type="button" class="btn btn-outline-secondary" data-bs-dismiss="modal">Cancel</button>
